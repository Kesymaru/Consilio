	...
	$.ajax({
		...
		success: function(response){
			...
			if( response.length > 0){
				...
				notificaError("Error: Proyectos.js Comentarios().<br/>"+response);
			}
		},
		fail: function(response){
			LoadingClose();
			notificaError("Error: AJAX FAIL Proyectos.js Comentarios().<br/>"+response);
		}
	});
}
			...
			if(response.length > 0){
				...
				$("#comentarios-articulos li").each(function(){
					...
					$(this).dblclick(function(){
						if( $.cookie('cargando') == "false"){
							$.cookie('cargando', true);
							ComentariosArticulo( $(this).attr('id') );
						}
					});
				});

			}else{
				notificaError("ERROR: Proyectos.js Comentario().<br/>"+response);
			}
		},
		fail: function(response){
			notificaError("ERROR: AJAX fail Proyectos.js Comentario().<br/>"+response);
		}
	});
	...
	$.ajax({
		...
		success: function(response){
			if(response.length > 3){
				...
				notificaError("Error: Proyectos.js ComentariosArticulo().<br/>"+response);
			}
		},
		fail: function(response){
			notificaError("ERROR: Proyectos.js ComentariosArticulo().<b/>"+response);
		}
	})
}
